...
class CameraOnEvenSphere(blender_nerf_operator.BlenderNeRF_Operator):
    '''Camera on Sphere Operator'''
    bl_idname = 'object.camera_on_even_sphere'
    ...
    def execute(self, context):
        ...
        error_messages = self.asserts(scene, method='CES')
        ...
        if scene.logs: self.save_log_file(scene, output_path, method='CES')
        ...
        if scene.test_data:
            # testing transforms
            output_data['frames'] = self.get_camera_extrinsics(scene, camera, mode='TEST', method='CES')
        ...
        if scene.train_data:
            ...
            # training transforms
            sphere_output_data['frames'] = self.get_camera_extrinsics(scene, sphere_camera, mode='TRAIN', method='CES')
            self.save_json(output_path, 'transforms_train.json', sphere_output_data)
            print("output path for transforms_train.json is ", output_path)
            ...
            if scene.render_frames:
                ...
                scene.frame_end = scene.frame_start + scene.ces_nb_frames - 1 # update end frame
