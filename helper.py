...
def visualize_camera_all(self,context):
    print("Reaches visualize camera_all!")
...
def visualize_camera(self, context):
    ...
    if CAMERA_NAME not in scene.objects.keys() and not scene.camera_exists:
        ...
        FIXED_FOV = 39.6
        bpy.data.cameras[CAMERA_NAME].angle_x = math.radians(FIXED_FOV)
...
def ces_camera_update(scene):
    ...
    if CAMERA_NAME in scene.objects.keys():
        ...
        if scene.upper_views:
            z = 1 - (i / (scene.ces_nb_frames))  # z ranges from 1 to -1
        else:
            z = 1 - (2 * i / (scene.ces_nb_frames))  # z ranges from 1 to -1
        theta = golden_angle * i           # Angle in the xy-plane
        x = math.sqrt(1 - z**2) * math.cos(theta) * scene.sphere_radius
        y = math.sqrt(1 - z**2) * math.sin(theta) * scene.sphere_radius
        z = z * scene.sphere_radius
        
        x += scene.sphere_location[0]
        y += scene.sphere_location[1]
        z += scene.sphere_location[2]
        print(x,y,z)
        scene.objects[CAMERA_NAME].location = mathutils.Vector((x,y,z)) # sample_from_sphere(scene)

# # update ces camera when changing frame
@persistent
